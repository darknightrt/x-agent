...
class OrchestratorAgent(BaseAgent):
    ...
    async def _create_execution_plan(
        ...
        # 按 sh.md：下线“综合市场分析报告 / 评论标签体系分析”，只保留“行业新闻简报”
        steps = [
            ExecutionStep(
                step_id="scrape_data",
    ...
    def _update_shared_context(
        ...
        if step_id == "scrape_data":
            ...
            analyses_payload = step_result.get("data", {}) or {}
            shared_context["posts_with_comments_analyses"] = analyses_payload
            summary = analyses_payload.get("summary", {}) or {}
            logger.info(
            ...
            # 为 Reporter 构造“简报所需”的最小 analysis 结构
            # 形状对齐 reporter_skills._build_briefing_data(): analysis["analysis"]["metadata"]
            relevant_posts = analyses_payload.get("relevant_posts", []) or []
            TOP_N = 10
            RELATED_N = 10
            ...
            def _total_engagement(post: Dict[str, Any]) -> int:
                # 仅基于收藏/分享/评论排序
                return (
                    int(post.get("collected_count", 0) or 0) * 2
                    + int(post.get("shared_count", 0) or 0) * 3
                    + int(post.get("comments_count", 0) or 0) * 3
                )
            ...
            for item in relevant_posts:
                ...
                total_comments_analyzed += int(analysis.get("comments_count", 0) or 0)
                ranked.append((post, analysis, _total_engagement(post)))

            # Hot: 仍按互动量 total_engagement 排
            ranked.sort(key=lambda x: x[2], reverse=True)
            for post, analysis, total_eng in ranked[:TOP_N]:
                top_posts.append({
            ...
            # Related: 剔除热点后，按互动量（归一化）排序
            hot_ids = {p.get("note_id") for p in top_posts if p.get("note_id")}
            max_eng = max([x[2] for x in ranked], default=0)
            denom = math.log1p(max_eng) if max_eng > 0 else 1.0

            def _norm_eng(total_eng: int) -> float:
                if total_eng <= 0:
                    return 0.0
                return 100.0 * (math.log1p(total_eng) / denom)

            related_ranked: List[tuple] = []
            for post, analysis, total_eng in ranked:
                note_id = post.get("note_id")
                if note_id and note_id in hot_ids:
                    continue
                final_score = float(_norm_eng(total_eng))
                related_ranked.append((post, analysis, total_eng, final_score))

            related_ranked.sort(key=lambda x: x[3], reverse=True)
            for post, analysis, total_eng, final_score in related_ranked[:RELATED_N]:
            ...
            shared_context["analysis"] = {
                "success": True,
                "analysis": {
                    # overall_score 等旧字段不再生成/不再需要
                    "metadata": {
                        "total_posts_analyzed": int(summary.get("total_posts", 0) or 0),
                        "relevant_posts": int(summary.get("relevant_count", 0) or 0),
                        "avg_engagement_score": float(summary.get("avg_engagement_score", 0) or 0),
                        "total_comments_analyzed": int(total_comments_analyzed),
                        "top_posts": top_posts,
