...
async def search_posts_skill(
    ...
    try:
        # 调用 X/Twitter (TikHub) MCP 服务器（内部实现走 Twitter Web API: fetch_search_timeline）
...
async def get_comments_skill(
    ...
    try:
        # 调用 X/Twitter (TikHub) MCP 服务器（内部实现走 Twitter Web API: fetch_post_comments）
        result = await agent.use_mcp(
            server_name="xhs",
...
async def batch_get_comments_skill(
    ...
    try:
        # 调用 X/Twitter (TikHub) MCP 服务器（串行批量拉评论，避免限流）
        result = await agent.use_mcp(
            server_name="xhs",
            ...
            if keyword_notes and comments_per_note > 0:
                ...
                note_ids = [
                    ...
                    if (
                        n.get("note_id")
                        and (
                            # TikHub 返回结构变动时 comments_count 可能缺失；缺失则尝试抓取一次
                            n.get("comments_count") is None
                            or int(n.get("comments_count", 0) or 0) > 0
                        )
                    )
                ]
