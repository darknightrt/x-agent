...
async def generate_keywords_skill(
    ...
    prompt = f"""
你是一位“每日行业新闻简报”选题与关键词分析专家。请为以下业务创意生成 {count} 个搜索关键词，用于推特中抓取“本期看点”和“热点帖子内容”所需的素材（越像新闻选题越好）。

业务创意："{business_idea}"

要求：
1. 关键词应该是用户在x会搜索的短语（更偏“热点/动态/选题”而不是纯产品词）
2. 每个关键词 3-6 个字
3. 覆盖不同角度（至少包含 1-2 个“热点类型”角度），包括：行业动态/趋势、政策与合规、新品/活动、用户需求变化、使用场景、人群等
4. 返回中文关键词
5. 如果业务创意中包含地点信息，请优先生成地域相关关键词（如“城市+品类/活动/推广”）

示例：
- 输入："在深圳卖陈皮"
- 输出：["深圳陈皮", "新会陈皮深圳", "陈皮养生深圳", "陈皮茶深圳活动", "深圳特产陈皮"]

输出要求（不要输出多余步骤、不要输出 JSON 文本、不要加代码块标记）。系统会按字段结构化解析，你只需要输出关键词列表字段 `keywords`（共 {count} 个）。
"""
...
async def refine_keywords_skill(
    ...
    prompt = f"""
你是一位“每日行业新闻简报”选题与关键词分析专家。请根据用户反馈优化以下关键词，用于抓取“本期看点 / 热点内容”的素材。
...
要求：
1. 优化后的关键词列表 `refined_keywords`：数量与原始关键词相同
2. 优化原因 `refinement_reason`：一句话即可，聚焦“更像新闻选题/更利于抓到热点内容”
3. 可选补充 `suggested_additions`：额外建议 1-2 个关键词（可为空）

输出要求：
- 不要输出多余步骤
- 不要输出 JSON 文本、不要加代码块标记
- 系统会按字段结构化解析，你只需要输出这 3 个字段：`refined_keywords`、`refinement_reason`、`suggested_additions`
"""
