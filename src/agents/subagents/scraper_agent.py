...
class ScraperAgent(BaseAgent):
    ...
    async def _batch_scrape(
        ...
        # orchestrator 会把 step.params 合并进 context 传入 execute()，
        # 因此这里需要优先从 context 读取参数，避免总是回退到默认值。
        pages_per_keyword = context.get("pages_per_keyword", kwargs.get("pages_per_keyword", 2))
        comments_per_note = context.get("comments_per_note", kwargs.get("comments_per_note", 20))
        max_notes = context.get("max_notes", kwargs.get("max_notes", 20))
